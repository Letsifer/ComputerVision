...
vector<double> RansacAlgorithm::findHomography(const vector<PointMatch> &matches) {
    ...
    for (int iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        auto homography = findCurrentHomography(matches);
...
vector<int> RansacAlgorithm::chooseMatches() {
    vector<int> choices;
    //TODO remake this choice
    for (int i = 0; i < K; i++) {
        choices.push_back(i);
    }
    return choices;
...
vector<double> RansacAlgorithm::findCurrentHomography(const vector<PointMatch> &matches) {
    auto choices = chooseMatches();
